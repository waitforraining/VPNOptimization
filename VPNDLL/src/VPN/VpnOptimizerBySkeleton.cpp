...
namespace ViewPointNetwork
{
	...
	OptSkelParam::OptSkelParam(string str)
	{
		...
		while (std::getline(iss, tmpStr, '-'))
		{
			...
			{
			case 't':
				houseType = HouseType(stoi(tmpStr.substr(1, tmpStr.size() - 1)));
				break;
			case 'c':
				cell = stod(tmpStr.substr(1,tmpStr.size() - 1));
				break;
			case 'r':
				stationRadiusMin = stod(tmpStr.substr(1,tmpStr.size() - 1));;
				break;
			case 'R':
				stationRadiusMax = stod(tmpStr.substr(1, tmpStr.size() - 1));
				break;
			case 'l':
				overlapThresh = stod(tmpStr.substr(1, tmpStr.size() - 1));
				break;
			case 's':
				scoreThresh = stod(tmpStr.substr(1, tmpStr.size() - 1));
				break;
			case 'd':
				denseStep = stod(tmpStr.substr(1, tmpStr.size() - 1));
				break;
			default:
				break;
			}
